...
uli_ode <- odin({
  ...
  deriv(Is) <- 
    Ia * sigma - 
    Is * (mutb + mu + self_cure + tx_initiation_delay) - 
    Is * r_incar + 
    Is_p * r_release *(1-screen_exit_s) 
  
  deriv(Tr) <-
    Is * tx_initiation_delay -
    Tr * tx_succes_rate -
    Tr * (1-tx_succes_rate) +
  ...
  deriv(Ia_p) <- 
    ...
    Tr_p * (1-tx_succes_rate)
  
  deriv(Is_p) <- 
    Ia_p * sigma_p - 
    Is_p * (mutb + mu + self_cure + tx_iniation_delay_p) + 
    Is * r_incar * (1-screen_entry_s) - 
    Is_p * r_release -
    Is_p * screen_yearly_s 
  
  deriv(Tr_p) <-
    Is_p * tx_iniation_delay_p -
    Tr_p * mu -
    Tr_p * tx_succes_rate -
    Tr_p * (1-tx_succes_rate) +
    Tr * r_incar -
  ...
  tx_initiation_delay <- if (time > 950) 1*0.87 else 0#       # Careseeking rate (1 year)
  tx_iniation_delay_p <- if (time > 950) 0.5  else 0#       # Careseeking rate (1 year)
  tx_succes_rate <- 0.65
